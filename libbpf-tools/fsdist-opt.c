...
#include "fsdist-opt.h"
#include "fsdist-opt.skel.h"
...
static int print_hists(struct fsdist_opt_bpf__bss *bss)
{
...
static int fentry_set_attach_target(struct fsdist_opt_bpf *obj)
{
...
static void disable_fentry(struct fsdist_opt_bpf *obj)
{
...
static void disable_kprobes(struct fsdist_opt_bpf *obj)
{
...
static int attach_kprobes(struct fsdist_opt_bpf *obj)
{
	long err = 0;
	struct fs_config *cfg = &fs_configs[fs_type];
...
int main(int argc, char **argv)
{
	...
	skel = fsdist_opt_bpf__open_opts(&open_opts);
	if (!skel) {
	 ...
	 */
	err = support_fentry ? fsdist_opt_bpf__attach(skel) : attach_kprobes(skel);
	if (err) {
