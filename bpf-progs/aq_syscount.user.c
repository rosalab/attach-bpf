...
int main(int argc, char *argv[])
{
    ...
    char * bpf_path = "aq_syscount.kern.o";
    char * entry = "sys_enter";
    char * exit  = "sys_exit";
    ...
    struct bpf_map * sys_data = bpf_object__find_map_by_name(prog, "data");
    ...
    bpf_program__set_color(program_entry, 0x0, 0x0);
    bpf_program__set_color(program_exit, 0x0, 0x0);
    ...
    if(!bpf_program__attach(program_entry)) {
        char err[256];
        libbpf_strerror(errno, err, 256);
        printf("Failed to attach sys_enter: %s\n", err);
    }
    if(!bpf_program__attach(program_exit)) {
        char err[256];
        libbpf_strerror(errno, err, 256);
        printf("Failed to attach sys_exit: %s\n", err);
    }
    ...
    int map_ret;
    for (int i = 0; i < 600; i++) {
        map_ret = bpf_map__lookup_elem(sys_data, &i, 4, &dat, sizeof(dat), 0);
        if (!map_ret) {
            printf("Syscall: %d Count: %llu Time: %llu\n", i, dat.count, dat.total_latency);
        }
    }
