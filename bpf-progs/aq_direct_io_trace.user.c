...
int main(int argc, char *argv[])
{
    ...
    char * bpf_path = "./aq_direct_io_trace.kern.o\0";
    ...
    // setup a pairwise libbpf_opts struct
    LIBBPF_OPTS(bpf_object_open_opts, open_opts);
    //open_opts.pw = true; // Martk this object as a pairwise object
    ...
    // setup the entry and exit info structs
    // set as entry or exit program (does this generalize?)
    //struct bpf_pw_info entry_info;
    //entry_info.pw_state = BPF_PW_ENTRY;
    //entry_info.pw_stack_size = 8; // For 64-bit timestamp
        
    //struct bpf_pw_info exit_info;
    //exit_info.pw_state = BPF_PW_EXIT;
    //exit_info.pw_stack_size = 8; // For 64-bit timestamp

    // get bpf_program structs
    ...
    //bpf_program__set_pw(program_entry, &entry_info);
    ...
    //if (! map_hist) {
    //    printf("Failed to get histogram map\n");
    //}
    ...
    //bpf_program__set_color(program_exit, 0x1, 0x2);
     ...
     */
    struct bpf_program * dio_trace = bpf_object__find_program_by_name(prog, "dio_trace");
    if (!dio_trace) {
        printf("Failed to find dio_trace program\n");
        return -1; 
    }
    bpf_program__attach(dio_trace);
