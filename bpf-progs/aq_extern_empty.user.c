...
int main(int argc, char *argv[])
{
    ...
    char *bpf_path; // = "aq_extern.kern.o";

    LIBBPF_OPTS(bpf_object_open_opts, open_opts);

    int type = atoi(argv[1]);
    if (type == 0) {
        bpf_path = "aq_empty.kern.o";
    }
    else if (type == 1) {
        bpf_path = "aq_extern.kern.o";
        //bpf_path = "aq_empty.kern.o";
        open_opts.pw = true;
    }
    else {
    ...
    //char * bpf_path = "extern_empty.kern.o";
    ...
    if (type == 0) {
        printf("With no opts\n");
        prog = bpf_object__open(bpf_path);
    }
    else if (type == 1) {
        printf("With opts\n");
        prog = bpf_object__open_file(bpf_path, &open_opts);
    }
    ...
    struct bpf_program * trace_sys_empty_enter = bpf_object__find_program_by_name(prog, "trace_sys_empty_enter");
    if (trace_sys_empty_enter == NULL) {
        printf("Enter is null\n");
    }
    struct bpf_program * trace_sys_empty_exit = bpf_object__find_program_by_name(prog, "trace_sys_empty_exit");

    if (type == 1) {
        bpf_program__set_pw(trace_sys_empty_enter, &entry_info);
        bpf_program__set_pw(trace_sys_empty_exit, &exit_info);
    }
    ...
    struct bpf_program * programs[2] = {
        trace_sys_empty_enter,
        trace_sys_empty_exit,
    };
    ...
    for (int i = 0; i < 2; i++) {
        ...
        bpf_program__attach_pw(trace_sys_empty_enter, trace_sys_empty_exit);
    }
